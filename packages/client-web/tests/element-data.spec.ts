...
import getElementData from '../src/element-data';
...
describe( 'Get Element Data', () => {
    ...
    it( 'Should get all parents from element', () => {
        const anchor = document.createElement( 'a' );
        ...
        expect( data.parents ).to.be.have.length( 2 );
        expect( data.parents[ 0 ].xpath ).to.be.equals( '/html/body' );
        expect( data.parents[ 1 ].xpath ).to.be.equals( '/html' );
    } );
} );
